...
export function useLessons() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(apiUrl(api.lessons.list.path), { credentials: "include" });
...
export function useLesson(slug: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const url = apiUrl(buildUrl(api.lessons.get.path, { slug }));
...
export function useCompleteLesson() {
  ...
  return useMutation({
    mutationFn: async ({ id, score }: { id: number; score?: number }) => {
      const url = apiUrl(buildUrl(api.lessons.complete.path, { id }));
